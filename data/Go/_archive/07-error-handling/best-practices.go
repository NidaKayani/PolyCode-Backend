...
func main() {
	...
	demonstrateAdditionalPractices()
}
...
func goodValidationError(name, email string) error {
	...
	if name == "" {
		errList = append(errList, "name is required")
	} else if len(name) < 2 {
		errList = append(errList, "name must be at least 2 characters")
	}

	if email == "" {
		errList = append(errList, "email is required")
	} else if !strings.Contains(email, "@") {
		errList = append(errList, "email must be valid")
	}

	if len(errList) > 0 {
		return fmt.Errorf("validation failed: %s", strings.Join(errList, ", "))
	}
...
func (ce CategorizedError) Error() string {
	return fmt.Sprintf("[%d] %v", ce.Category, ce.Err)
}
...
func demonstrateAdditionalPractices() {
	...
	result, runErr := processWithGracefulDegradation()
	if runErr != nil {
